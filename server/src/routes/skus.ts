...
skusRouter.get("/bulk/template", requireRole("OWNER", "ACCOUNTANT", "WAREHOUSE"), (_req, res) => {
...
skusRouter.post("/bulk/preview", requireRole("OWNER", "ACCOUNTANT", "WAREHOUSE"), async (req, res) => {
...
skusRouter.post("/bulk/commit", requireRole("OWNER", "ACCOUNTANT", "WAREHOUSE"), async (req: AuthedRequest, res) => {
  const parsed = bulkRequestSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: "Invalid request body", details: parsed.error.flatten() });
